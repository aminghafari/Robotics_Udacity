...
def compute_color_histograms(cloud, using_hsv=True):
    ...
    hist_features = np.concatenate((ch1_hist[0], ch2_hist[0], ch2_hist[0])).astype(np.float64)
...
def compute_normal_histograms(normal_cloud):
    ...
    nx_hist = np.histogram(norm_x_vals, bins=32, range=(0, 1))
    ny_hist = np.histogram(norm_y_vals, bins=32, range=(0, 1))
    nz_hist = np.histogram(norm_z_vals, bins=32, range=(0, 1))
    # TODO: Concatenate and normalize the histograms
