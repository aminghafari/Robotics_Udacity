...
# Exercise-2 TODOs:
    ...
    data_filtered = outlier_filter.filter()

    # TODO: Voxel Grid Downsampling
    vox = data_filtered.make_voxel_grid_filter()
    LEAF_SIZE = 0.005
    ...
    white_cloud = XYZRGB_to_XYZ(extracted_outlier)
...
# Exercise-3 TODOs:
    ...
    for index, pts_list in enumerate(cluster_indices):
        ...
        pcl_cluster = extracted_outlier.extract(pts_list)
...
def pr2_mover(object_list):
    ...
    for i in range(0, len(object_list_param)):
        ...
        for j, iname in enumerate(labels):
            if iname == name:
                pick_pose = centroids[j]

        # TODO: Create 'place_pose' for the object
        place_pose = centroids[0]
...
#
 #       except rospy.ServiceException, e:
  #          print "Service call failed: %s"%e
    ...
    send_to_yaml('output_1', dict_list)
